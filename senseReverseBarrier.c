...
void central_barrier(int *localSense) 
{
...
int main(int argc, char** argv)
{
	if (argc==3)
	{
		...
		printf("METHOD -- SENSE_REVERSE (Thread# = %d, Barrier# = %d) -- Average time taken by threads to complete = %f s\n\n",MAXTHREADCOUNT,MAXBARRIERCOUNT,netVal/MAXTHREADCOUNT);
		return 0;
	}
	return 1;
}